...
if _G.BloxyScriptsUnload then pcall(_G.BloxyScriptsUnload) end
...
if type(SkinsLib) ~= "table" then error("[BloxyScripts] failed to load Skins lib") end
local GetWeaponProperties = safeRequire(RS.Components.Common.GetWeaponProperties)

-- ===================== КОНФИГ =====================
local CFG_FILE = "BloxyScripts.json"
local CFG_FILE_OLD = "SoloScripts.json"
local CFG = {
    enabled = true,
...
if type(ViewmodelMod) == "table" and type(ViewmodelMod.new) == "function" then
    ...
    ViewmodelMod.new = function(comp, weapon, skin, inspecting)
        if CFG.enabled and CFG.knifeModel and type(weapon) == "string" and isKnife(weapon)
            ...
            if ok and res then return res end
            warn("[BloxyScripts] knife " .. w .. " failed to build, falling back: " .. tostring(res))
        end
...
local function stroke_ui(i, col, th) local s = Instance.new("UIStroke") s.Color = col or T.stroke s.Thickness = th or 1 s.Parent = i return s end
...
local old_gui = hui:FindFirstChild("BloxyScripts") or hui:FindFirstChild("SoloScripts") or hui:FindFirstChild("LuxSkins")
if old_gui then old_gui:Destroy() end

local gui = Instance.new("ScreenGui")
gui.Name = "BloxyScripts"
...
local ttl = label(top, "BloxyScripts", 15, T.text, Enum.Font.GothamBold)
ttl.Size = UDim2.new(0, 105, 1, 0) ttl.Position = UDim2.new(0, 34, 0, 0)
local ttl2 = label(top, "BloxyStrike", 13, T.dim, Enum.Font.GothamMedium)
ttl2.Size = UDim2.new(0, 80, 1, 0) ttl2.Position = UDim2.new(0, 142, 0, 0)

local pageTabs, currentPage = {}, "skins"
local pageBar = Instance.new("Frame")
pageBar.Size = UDim2.new(0, 225, 0, 28) pageBar.Position = UDim2.new(0, 230, 0.5, -14)
...
local function buildSkins(weapon)
    ...
    for i, s in ipairs(list) do
        ...
        local st = stroke_ui(card, (sel == s.name) and T.accent or T.stroke)
...
_G.BloxyScriptsUnload = function()
    ...
    _G.BloxyScriptsUnload = nil
    _G.BloxyScripts = nil
end

_G.BloxyScripts = { CFG = CFG, reequip = reequip, skinsFor = skinsFor }

print("[BloxyScripts] loaded: " .. #allWeapons .. " weapons. RightShift — toggle menu.")