-- =========================================================
-- АДДОН: Auto-Lock Aimbot + ESP для Custom Hub
-- =========================================================
local Hub = getgenv().CustomHub
if not Hub then
    warn("Сначала запусти главный хаб!")
    return
end
...
EspTab:AddSection("Визуальные функции")
EspTab:AddToggle("Боксы (Box ESP)", "EspBox", false, function(s) state.esp_box = s end)
EspTab:AddToggle("Здоровье (HP)", "EspHp", false, function(s) state.esp_hp = s end)
EspTab:AddToggle("Дистанция (Distance)", "EspDist", false, function(s) state.esp_dist = s end)
EspTab:AddToggle("Трейсеры (Линии)", "EspTracers", false, function(s) state.tracers = s end)
local AimTab = Hub:CreateTab("Aimbot")
AimTab:AddSection("Настройки Auto-Lock")
AimTab:AddToggle("Включить Aimbot", "AimActive", false, function(s) state.aim = s end)
AimTab:AddToggle("Проверка стен (WallCheck)", "AimWallCheck", false, function(s) state.wall_check = s end)
AimTab:AddToggle("Показывать FOV", "AimFovVis", false, function(s) state.fov_visible = s end)
AimTab:AddSlider("Радиус FOV", "AimFovRad", 50, 400, 180, function(val) state.fov = val end)

print("Аддон успешно загружен!")